...
class BezierCurve:
    ...
    def calculate_bezier_points(self, points, current_iteration):
        ...
        if current_iteration < self.num_iterate:
            # Hanya perlu satu array baru untuk menampung titik-titik tengah
            mid_points = []
            for i in range(len(points) - 1):
                mid_point = self.mid_point(points[i], points[i + 1])
                mid_points.append(mid_point)
            
            # Rekursi ke bagian kiri dan kanan
            self.calculate_bezier_points(mid_points, current_iteration + 1)
            if current_iteration == 0:
                self.bezier_points.append(points[0])
            self.bezier_points.append(mid_points[-1])
            if current_iteration == 0:
                self.calculate_bezier_points(mid_points[::-1], current_iteration + 1)
                self.bezier_points.append(points[-1])
    ...
    def draw_animate(self, fig, ax):
        ...
        ax.set_xlim(min(ctrl_x) - 10, max(ctrl_x) + max(ctrl_x) // 10)
        ax.set_ylim(min(ctrl_y) - 10, max(ctrl_y) + max(ctrl_x) // 10)
        ax.set_xlabel('X')
        ax.set_ylabel('Y')
